...
$(document).on("click", "#menpoleras", function (e) {
  ...
  var p = "";
  if (n2n === 1) {
    //alerta si hay modificacion se deben modificar todos
    p =
...
$(document).ready(function () {
  ...
  $("input[list='evets']").focusout(function () {
    //para llenar sw de evento
    var cli = this.value;
    if (cli !== "") {
      $.ajax({
  ...
  $("input[list='clos']").focusout(function () {
    //para llenar cliente
    var cli = this.value;
    if (cli !== "") {
      $.ajax({
  ...
  $("input[list='dis']").focusout(function () {
    //para llenar sw de destino
    var cli = this.value;
    if (cli !== "") {
      $.ajax({
        url: "../plantilla/ajax.php",
