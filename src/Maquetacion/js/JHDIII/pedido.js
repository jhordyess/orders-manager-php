...
$.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
  ...
  if ((isNaN(min) && isNaN(max)) || (isNaN(min) && mydate <= max) || (min <= mydate && isNaN(max)) || (min <= mydate && mydate <= max)) {
    return true;
  }
  return false;
});
$(document).ready(function () {
  var tabla = $('#registros').DataTable({
    ...
    //l opcion de cantidad de tuplas a mostrar
    //r preprocesing????
    //f es el filtro general
    //t es la tabla...si no se dice no se hace hacia abajo
    //i la informacion
    //p las next page
    "columns": [
      ...
      {"orderable": true},
      {"orderable": true},
      {"orderable": true,type: 'date-eu'},//fecha partida
    ...
    select: {
      ...
      buttons: [{
          ...
          extend: "selected",
          text: '<i class ="fas fa-calendar-times"></i> Deuda',
          action: function (e, dt, button, config) {
            iterativo(dt, '3');
          }
        }, {
          ...
          extend: "selected",
          text: '<i class ="fas fa-check"></i> Entregado',
          action: function (e, dt, button, config) {
            iterativo(dt, '2');
          }
        }, {
          ...
          action: function (e, dt, button, config) {
            var x = window.confirm("¿Realmente desea eliminar registros seleccionados?");
            if (x) {
              iterativo(dt, '0');
            }
          }
          ...
          action: function (e, dt, button, config) {
            var idpedido = -1;
            idpedido = numeral(dt.row('.selected').data()[1]);
            window.open('../../Reporte/LaTeX/Recibo.php?a=' + idpedido, '_blank');
          }
        }, {
          ...
          action: function (e, dt, button, config) {
            var idpedido = numeral(dt.row('.selected').data()[1]);
            $.ajax({
          ...
          action: function (e, dt, button, config) {
            ...
            window.open('../../Reporte/LaTeX/Itinerario.php?v=' + JSON.stringify(vector), '_blank');
          }
        }
      ]
      ...
      // Update footer
      $(api.column(6).footer()).html('&Sigma;: ' + pageTotal + ' ('+allTotal + ' en total)');
    }
  });
    ...
    $.ajax({
      ...
      success: function (output) {
        if (output.charAt(0) == 'X') {
          ...
          alert(output);
          window.open('frm_listar.php', '_self');
        }
      }
    });
  }
  ...
  $(".emi").change(function () {
    ...
    switch (t) {
      ...
      case '4':
        //hacia atras
        n = new Date();
        var za=n.getFullYear();
        m = new Date("January 1, "+za+" 00:00:00");
